...
window.scrollTo({
    ...
    // === Wiederaufnahme letztes Kapitel (nicht-invasiv) ===
(function setupLastPageRestore() {
    const STORAGE_KEY = "af_last_page";
  
    // Prüfen, ob eine Seite in der Kapitel-Liste erlaubt ist
    function isValidChapter(href) {
      return typeof href === "string" && chapters.includes(href);
    }
  
    // 1) Beim ersten Laden: falls vorhanden, letzte Seite statt home.html laden
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved && isValidChapter(saved) && saved !== 'home.html') {
        // optional: direkt merken (falls Seite neu geöffnet wurde)
        localStorage.setItem(STORAGE_KEY, saved);
        // Ruft deine bestehende Funktion auf – unverändert
        loadContent(saved);
      }
    } catch (e) {
      // localStorage evtl. nicht verfügbar – dann einfach schweigen
    }
  
    // 2) Klicks auf Links abfangen (Capture-Phase), Ziel merken – bestehende Listener bleiben erhalten
    ...
    }, true); // <-- capture: true, läuft vor deinen anderen Click-Handlern
  
    // 3) Optional: Fallback für programmatische Navigationsaufrufe ohne Klicks
    //    Wenn andere Stellen loadContent(...) aufrufen, kannst du das hier ohne Änderung "mitloggen",
    //    indem du einmalig eine leichte Wrapper-Logik setzt – Verhalten bleibt identisch.
    if (typeof window.loadContent === "function") {
      const __origLoadContent = window.loadContent;
      window.loadContent = function(page) {
        if (isValidChapter(page)) {
          try { localStorage.setItem(STORAGE_KEY, page); } catch (e) {}
        }
        return __origLoadContent(page); // unverändertes Verhalten
      };
    }
  })();
